...
test('test4.spec.ts', async ({ page }) => {
    ...
    await test.step("Mở Personal Notes Page", async () => {
        await personalNotesPage.openMaterialPage();
        await personalNotesPage.gotoPage("Bài học 4: Personal notes");
    });
    ...
    await test.step("Thêm mới 10 note", async () => {
        ...
        await personalNotesPage.addNotes(notes);
    });
    const keyword = 'Apple bán được ba tỷ iPhone';

    await test.step("Search theo tiêu đề bài báo bất kì", async () => {
        await personalNotesPage.searchNotes(keyword);
    });

    await test.step("Kiểm tra kết quả tìm kiếm có chứa keyword", async () => {
        await personalNotesPage.verifyAllSearchResultsContain(keyword);
    });
